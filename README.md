...
## Member / Role
* 임현수 / 범죄데이터 전처리 및 탐색
* 정다은 / 데이터 수집, 데이터 전처리, PPT 작성, ReadMe 작성, 범죄 발생과 지구대/생활권공원면적 상관관계 탐색

## 조사 동기
* '강남 3구 (강남구, 서초구, 송파구)가 범죄에 대해 안전하지 않다'는 분석 결과에 의문을 가지고, 구별 인구 수에 따른 조정이 필요할 것이라고 판단
* 범죄발생과 유의한 상관성을 보이는 요소가 있다면, 서울시 각 자치구의 치안 발전에 기여할것이라 예상
